...
use bevy_egui::egui::{Id, InnerResponse, Response, Sense, Ui};
...
pub fn project_list(ui: &mut Ui, projects: &ExistingProjects) -> EResult<Option<ProjectListAction>> {
    if project_row(ui, "➕", "Create a new project", None, false).is_some() {
        ...
        let path = OsString::from("/home/grabn/projects/bevytor/das_demo");
        let name = "Das demo".to_string();
        if let Err(e) = Project::verify_new(path.clone()) {
            bail!("PROJECT::LIST::CREATE", e);
        }
        return Ok(Some(ProjectListAction::Create(ProjectDescription{name, path})))
    }
    ui.separator();
    ...
    if project_row(ui, "🗁", "Open an existing project", None, false).is_some() {
        ...
        return Ok(Some(ProjectListAction::NewOpen(ProjectDescription{name, path})))
    }
    ...
    for project in &projects.0 {
        if let Some(action) = project_row(ui, "🚀", &project.name, project.path.as_os_str().to_str(), true) {
            return match action {
                ProjectRowAction::Select => Ok(Some(ProjectListAction::ExistingOpen(project.clone()))),
                ProjectRowAction::Remove => Ok(Some(ProjectListAction::ExistingRemove(project.clone())))
            }
        }
    }
...
fn project_row(ui: &mut Ui, icon: &str, name: &str, path: Option<&str>, removable: bool) -> Option<ProjectRowAction> {
